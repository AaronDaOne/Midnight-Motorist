...
CAS_F_SPR = pg.transform.smoothscale(
	pg.image.load(src.CAS).convert_alpha(),
	(SCREEN_SIZE[0] * 0.07, SCREEN_SIZE[0] * 0.07)
)
CAS_B_SPR = pg.transform.rotate(CAS_F_SPR, 180)
...
LAUGH_SOUND = pg.mixer.Sound(src.SFX_LAUGH)
...
def set_volume(amount):
	pg.mixer.music.set_volume(amount)
	CAR_SOUND.set_volume(amount)
	HIT_SOUND.set_volume(amount)
	LAUGH_SOUND.set_volume(amount)
	COUNTDOWN_SOUND.set_volume(amount)
	GO_SOUND.set_volume(amount)
...
def change_volume(amount):
	...
	LAUGH_SOUND.set_volume(LAUGH_SOUND.get_volume()+amount)
...
def draw(chr_loc, bg_scroll, npc_list:list[Npc], speed_txt, score_txt, hitbox, fps_txt, lives, spr_hit_count):
	...
	for npc in npc_list:
		if npc.g:
			WIN.blit(CAS_F_SPR if npc.is_forward else CAS_B_SPR, npc.pos - NPC_SPR_OFFSET)
		else:
